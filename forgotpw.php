...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
	if(trim($_POST['email']) == '' || trim($_POST['login']) == '') {
		...
		if($_POST['pass'] == $_POST['pass2']) {
			if (filter_var($_POST['email'], FILTER_VALIDATE_EMAIL)) {
				...
				//$password = $_POST['pass'];
				$email = $_POST['email'];
				$ret = $anope->DoCommand("NickServ", "$user", "RESETPASS $user $email");
				$searchword = 'Onjuist e-mailadres';
				$matches = array();
					foreach($ret as $k=>$v) {
...
<script src="https://www.google.com/recaptcha/api.js?render=6Ld6Y4AUAAAAAH6uCWtHPw9psZxn8qZ7Rqy2ysWL"></script>
    <script>
		...
		<title>..::Chattersworld Chameleon::.. Wachtwoord Herstel</title>
    </head>
                    ...
                    <div class="col-12 d-flex align-items-center justify-content-center">
                        ...
                        if(isset($_POST['login'])) {
                            ...
                            if(isset($success)) {
								...
								echo '<button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button></div>';
								echo '<meta http-equiv="refresh" content="3; url=resetpass.php?login='.$_POST['login'].'" />';
                                    ...
                                    <div class="card-body">
									<div class="alert alert-warning alert-dismissable"><strong>Let op!</strong> Bij het invullen van uw gegevens krijgt u vervolginstructies per e-mail, deze kunnen in uw SPAM map terecht komen!
								</div>
				...
				function CheckSpace(event)
				{
					if(event.which ==32)
					{
						...
						return false;
                                                ...
                                                <input type="text" name="login" class="form-control round" id="user-name" pattern="^[a-zA-Z0-9][a-zA-Z0-9-_]{2,}$" onkeypress="CheckSpace(event)" placeholder="Nicknaam" required>
                                                <div class="form-control-position">
                                                    <i class="ft-user"></i>
                                                </div>
                                            </fieldset>
                                            ...
                                            <div class="form-group text-center">
                                                ...
                                                <button type="submit" class="btn round btn-block btn-glow btn-bg-gradient-x-purple-blue col-12 mr-1 mb-1">Herstel uw wachtwoord!</button>
